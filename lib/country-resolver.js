...
function resolveGCC (params) {
  ...
  if (params.rdsPi) {
    broadcastCountryId = params.rdsPi.substring(0, 1)
  } else if (params.dabSid && params.dabSid.length === 4) {
    broadcastCountryId = params.dabSid.substring(0, 1)
  } else if (params.dabSid && params.dabSid.length === 8) {
    broadcastCountryId = params.dabSid.substring(2, 3)
    params.ecc = params.dabSid.substring(0, 2)
  } else {
    throw new Error(
      'RDS Programme Identification (rdsPi) OR DAB Service Identifier (dabSid) must be set before attempting to resolve'
    )
  }
...
function resolveGCCWithECC (ecc, broadcastCountryId) {
  ...
  if (!country) {
    throw new Error(
      'A Global Country Code (GCC) could not be resolved for the given input. No match found in county data.'
    )
  }
...
function resolveGCCWithCountryCode (isoCountryCode, broadcastCountryId) {
  ...
  if (results.length === 0) {
    throw new Error(
      'A Global Country Code (GCC) could not be resolved for the given input. No match found in country data.'
    )
  }
